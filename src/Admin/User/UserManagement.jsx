...
const AdminUserManagement = () => {
  ...
  useEffect(() => {
    const fetchAllMasterData = async () => {
      try {
        const [
          commRes, countryRes, stateRes, cityRes, eduRes, occRes, mtRes, starRes, moonRes
        ] = await Promise.all([
...
const UserDetailModal = ({ user, onClose, onUpdateSuccess, masterData }) => {
  ...
  // --- EXACTLY MATCHES USER DASHBOARD LOGIC ---
  const handleNewPhotoChange = async (e) => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        // Compress the file natively like UserDashboard does. 
        // No risky 'new File()' conversions needed.
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Fix applied here: Use a fallback filename if it got lost in compression
          ...
          <div className="um-edit-form">
            ...
            <div className="um-form-grid" style={{marginBottom: '20px'}}>
              ...
              <FormGroup label="City">
                ...
                <datalist id="edit-city-list">{masterData?.cities.map((c, idx) => <option key={idx} value={c.name} />)}</datalist>
              </FormGroup>
              ...
              <FormGroup label="State">
                ...
                <datalist id="edit-state-list">{masterData?.states.map((s, idx) => <option key={idx} value={s.name} />)}</datalist>
              </FormGroup>
